...
def main():
    ...
    parsed = parse_formula(formula, periodic_table)
    
    molar_mass = compute_molar_mass(parsed, periodic_table)
...
def compute_molar_mass(symbol_quantity, periodic_table_dict):
    p = symbol_quantity

    total_mass = 0
    
    #identify elememt and number of elements
    for i in p:
        #Element symbol
        e = i[0]
        #number of elements
        ...
        #look for te element mass
        for key, value in periodic_table_dict.items():
            #if the symbol in make periodditc_table == my symbol
            if key == e:
                #multiply the elements's weoht by the number elements
                mass = (value[1] * m)
                total_mass += mass
    return total_mass
